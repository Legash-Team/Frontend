      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative group">
            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-soft/40 group-focus-within:text-crimson" size={18} />
            <input 
              type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="contact@hospital.org"
              className="w-full h-13 pl-12 pr-4 bg-paper border border-line-soft rounded-md outline-none focus:border-crimson text-sm"
            />
          </div>
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-[11px] font-mono font-bold text-ink-soft uppercase tracking-wider">Password</label>
          <div className="relative group">
            <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-soft/40 group-focus-within:text-crimson" size={18} />
            <input 
              type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)}
              placeholder="••••••••"
              className="w-full h-13 pl-12 pr-12 bg-paper border border-line-soft rounded-md outline-none focus:border-crimson text-sm"
            />
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-ink-soft/40 hover:text-ink">
              {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
            </button>
          </div>
          <div className="flex justify-end">
            <button type="button" onClick={() => setForgotModalOpen(true)} className="text-[10px] font-bold text-crimson hover:underline uppercase tracking-tighter">
              Forgot Password?
            </button>
          </div>
        </div>
        ...
        <div className="pt-4 space-y-6">
          <Button type="submit" variant="primary" isLoading={isLoading} className="w-full h-14 font-bold rounded-md shadow-lg">Log In</Button>
          <p className="text-center text-sm text-ink-soft">
            New facility? <Link to="/register" className="text-crimson font-bold hover:underline underline-offset-4">Register Hospital</Link>
          </p>
        </div>
      </form>
